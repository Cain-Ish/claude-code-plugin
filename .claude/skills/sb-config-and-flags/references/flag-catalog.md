...
| `SB_PLAN_FIRST_NUDGE` | `on` | Master switch for plan-first-nudge.sh: with the intent spine on it hosts Gates A/B + the plan→implement phase flip; with `SB_INTENT_SPINE=off` it degrades to the original soft once-per-session advisory (additionalContext + allow — never blocks). | KS | plan-first-nudge.sh | bash |
| `SB_PLAN_FIRST_FILES` | `2` | Distinct-code-file count that triggers Gate A (or the advisory when the spine is off). | TUNE | plan-first-nudge.sh | bash |
| `SB_PLAN_FIRST_MIN_LINES` | `3` | Edits below this line count never count a file toward the threshold. | TUNE | plan-first-nudge.sh | none |
| `SB_INTENT_SPINE` | `on` | Session Intent Spine master switch: per-prompt frozen-goal line (persona-context.sh), phase tracking (plan-first-nudge.sh + persona-tool-guard.sh), Gate A plan-before-implement deny-once, Gate B drift warn-then-deny-once, Gate C goal-quoting in the Stop verify block. Off = legacy advisory nudge, no goal line, no phase file. All spine paths fail OPEN. | KS | plan-first-nudge.sh + persona-context.sh + persona-tool-guard.sh + stop-verify-gate.sh | bash |
| `SB_DRIFT_FILES` | `4` | Consecutive zero-goal-keyword-overlap code files before Gate B warns (first set) / denies once (second set). On-scope files reset the run. | TUNE | plan-first-nudge.sh | bash |
| `SB_VERIFY_GATE` | `on` | Stop-hook verification gate. | KS | stop-verify-gate.sh:13 | bash |
| `SB_HOOK_PROFILE` | unset | `minimal` collapses the hook surface to essentials in one lever: defaults SAR banner, plan-first nudge, intent spine, dream autostage, critic offer, loop-dead banner, codemap orient, injection scan, and config-change audit to off (individually-set values always win). Guards, extraction, and session-load stay on. | profile | lib.sh (profile block) | bash |
...
| `SB_DREAM_CREATE_TIMEOUT_MS` | `300000` (5 min) | Wall clock for `dream-snapshot.sh` via MCP `dream_create`. Was 30s, which a ~950-page wiki copy + transcript selection overran under load on Windows; the killed script kept running and still wrote a pending dream. A timeout now reports "timed out after Ns" plus what the kill left behind (Windows: the snapshot may still finish; POSIX: a drm_* folder without status.json). Integer 30000–2147483647, else the default. | TUNE | mcp/src/tools/dream.ts | vitest |
...
| `SB_BUDDY_NOW` | empty | Pins the renderer clock (epoch seconds) so frame selection is deterministic. | TESTDBL | buddy-statusline.sh | bash |
