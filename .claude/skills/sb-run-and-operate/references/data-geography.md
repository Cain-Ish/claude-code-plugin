...
| `cost-router-events.jsonl` | cost-router routing-event ledger (only exists with the cost-router subplugin) | cost-router hooks | `cost-router-capture.sh` Stop hook → `cost-routing-patterns` wiki page | safe (loses routing telemetry) |
