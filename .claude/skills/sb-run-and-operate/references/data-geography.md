...
| ~~`tool-registry.json`~~ | REMOVED — its producer `discover-tools.sh` was deleted, so any copy on disk is frozen at whenever it last ran. `skills/status` correctly reports it as gone; do not reinstate a reader. | (no producer) | none | n/a |
