...
The comment-archaeology sweep: shipped code stops narrating its own history. ~160 changelog-style comments (version tags, dates, plan-doc paths, task codenames) across scripts/, hooks/, mcp/src, skills/, agents/, and cost-router/ were rewritten to state only the live constraint — incident + mechanism + fix-by-construction — or deleted where nothing non-obvious remained. Versions, dates, and plan paths now live in CHANGELOG/git-blame, where blame can date them; comments carry only what the next reader needs.

- **Doctrine (now codified in sb-docs-and-writing §8)**: keep the WHY (the WSL-bash shadow trap, the jq-CRLF discipline, the deadlock-every-future-dream warning — all preserved verbatim); keep stable cross-reference IDs where they are live join-keys (R1–R9, HOOK-5, G-HOOK-2 — verified against the scripts/tests/skills that grep for them); strip the archaeology ('was X now Y', 'since 0.24.45', 'P1.2 Task 2', 'R5.1 (CR-006)'). Three adversarial skeptic agents re-checked every delete/rewrite before execution and forced 11 reclassifications protecting constraint-carrying comments (e.g. lib.sh's deliberately-missing marker-clear helper, the R7 liveness-gate heading, the codemap Plan:-path headers — 2 of only 2 kept plan pointers).
- **Doc-truth fixes** (the audit's residual-drift ledger, closed): 5 dev skills corrected — `sb-config-and-flags` no longer denies the existence of `auto_codemap` (live at `extract-drain.sh:368` since 0.33.34, tested); `sb-proof-and-analysis-toolkit`/`sb-external-positioning`/`sb-research-methodology`/`sb-failure-archaeology` no longer claim P3a is plan-only (Phases 1–3 shipped 0.33.33–0.33.35). Two plan docs told the opposite of the truth and now don't: the P1 observability plan is checkbox-complete with two shipped-form deviation notes (utilization fold lives in stop-extract.sh; the `sb status` cumulative ratio was NOT built and stays open), and the loop-engineering spec's stale '"P0.5 next"' status note is annotated + superseded by a 2026-07-14 update.
- **Expected-skips manifest ARMED on Windows** (`tests/run-all.sh`): the MINGW/MSYS/CYGWIN arm pins the observed skip set (`test-dream-accept-guards test-dream-lifecycle test-symlink-guard`) — an unexpected whole-file SKIP now fails the suite on the platform where the local run IS the release gate. Linux/Darwin arms stay warn-only (never measured). `SB_EXPECTED_SKIPS` still overrides. Its meta-test (`test-run-all-skip-semantics`) now pins its own expectation via `SB_EXPECTED_SKIPS` so skip CLASSIFICATION stays hermetic under armed manifests — caught by the armed suite's own first run.
- Comment-only otherwise: no behavior change for any healthy install; `mcp/dist` rebuilt because bundle comments track src. Executed by 11 parallel subagents on disjoint file sets, each edit `bash -n`/`jq .`-verified, integrator-verified diff (every changed line comment/prose except the run-all arm and the plan-doc checkboxes).

## 0.33.39

The state-of-the-art pass, tranche 2 (closes the 62-agent audit's implementable findings): runtime state gets lifecycle discipline, the test suite gets economics + a false-green lock, and the anti-gaming gate gets the two precision fixes its own live false-positives demanded.

- **Gate precision (dogfooded)**: the P0.5 anti-game scan is now ARGUMENT-scoped — commands split on `&&`/`;`/`|` inside jq (same regex engine on BSD/GNU), and only a span whose OWN first token is `rm`/`git rm` with a test-shaped argument flags. The live false positive (`bash tests/test-x.sh … && rm -rf "$TMP"` on one chain) is now a regression fixture; the true positive on a chain (`rm tests/broken.test.ts && echo done`) still catches. And it's WINDOW-scoped via a dedicated `.verify-gate-agseen-<sid>` marker (separate from stop-extract's extraction marker — ownership documented), so one historical flag can't re-fire every Stop and exhaust the 2-block valve.
- **State/GC hygiene**: one 7d sweep now covers `.injected-manifest-*` + `.verify-gate-blocks-*` + `.critic-offer-*`; `utilization-counts.json` ages out 180d-idle keys; `.extraction-state.jsonl` prunes rows whose transcript is gone (torn-row tolerant, atomic); `*.bak.*` backups join the retention TTL; the wiki `.embeddings-cache.json` evicts entries for deleted pages (its own test caught a Windows jq-CRLF poisoning during development — `tr -d '\r'` on keys); `.rejected-extractions.jsonl` retired — rejections are bounded audit-log TRACE rows now; write-only `tool-registry.json` machinery DELETED (SessionStart hook entry + `discover-tools.sh` + its test — nothing ever read it); dead `regressions/` scaffold removed; dream creation refuses (loudly, deleting nothing) when ≥3 completed dreams sit unreviewed — each holds a full wiki staging copy.
- **Test economics**: `test-validate-plugin.sh` stubs the ~48 `claude` CLI spawns (one real-CLI case kept) — ~34s → single digits; `run-all.sh` gains a per-platform expected-skips manifest (unexpected SKIP fails the suite where the manifest is explicit — the false-green class gets its machine lock; warn-only where unobserved); permanent-SKIP `test-hooks-regression.sh` deleted; `mcp/test/` folded into src-co-located suites (unique cases preserved) and dropped from the vitest include; vitest setupFiles env snapshot/restore kills the cross-file env-bleed REASSERT hacks; extractor-backend fixed sleeps → bind-polls; `test-bundle-current.sh` now FAILS on orphaned committed bundles (previously never compared).
- Surface budget RATCHETED DOWN: scripts 52→51, tests 157→155.
- Attestation correction (loud, per house convention): the release commit's gates line claimed run-all ALL GREEN, but the pre-push run was actually 151/1 — test-exec-bits correctly failed on a mid-staging index/disk race (concurrent agents had deleted tests from disk before git add). Verified clean post-commit: test-exec-bits ALL PASS on the committed tree, full run-all ALL GREEN (155 tests) on re-run, CI green on both lanes. Integrator lesson recorded: gate releases on the suite's EXIT CODE, never a log tail. Executed by two Opus subagents (state/GC + tests/gate) with integrator verification; one earlier attempt died pre-write on a session cap and was cleanly re-run.

## 0.33.38

The state-of-the-art pass, tranche 1 of the 62-agent optimize/cleanup audit (5 domain auditors → per-finding adversarial verification → synthesis): the bash hot path loses ~85% of its process spawns, the TS layer loses its 23 frontmatter copies and dual error grammars, and every skill/agent doc now tells the truth about shipped behavior. Implemented by parallel subagents (bash/TS/docs) with the integrator verifying each report.

- **Hot path (measured)**: `persona-tool-guard.sh` ~1.3s → ~0.4s per tool call — payload fields parse in ONE line-protocol jq (NOT @tsv, which backslash-escapes Windows paths and regex strings), and the rules loop is a single sentinel-framed jq stream instead of 1+5N spawns. Found live while rewriting: `$(...)` trailing-newline stripping silently dropped the LAST rule's frame (warn-rm-rf never evaluated) — non-empty sentinel token + probe-verified. `kb-schema.sh` 10 jq → 1 (~330ms off EVERY hook via the unconditional lib.sh source). `flow-guard.sh` + `tool-return-scanner.sh` (measured 1152ms) same treatment. `ensure-dirs.sh` full-wiki validate now 24h-throttled (`.last-ensure-validate` stamp). `cost-router-capture.sh` checks its no-op condition BEFORE paying the lib.sh tax. `session-load.sh` dream/health/episodic jq clusters batched (5→1, 4→1, 2→1; the health fold also fixes the old H_STATUS CR-taint). persona-tool-guard is now hook-timer-wrapped (budget 5) so its latency is finally telemetered — only that guard, to avoid audit-log flood.
- **lib.sh funnels + locks**: `sb_mtime()` (10 sites migrated; fail-OPEN sites deliberately kept inline and documented) and `sb_knowledge_dir()` (option > env > default, matching brain-paths.ts; ~20 sites migrated, 12-file allowlist for pre-lib.sh/standalone resolvers) with a NEW drift-guard in `test-kb-schema.sh` banning hand-rolled resolution. Kill-switch idiom normalized at 4 default-on sites that silently disabled on any non-`on` value.
- **TS layer**: new dep-free `frontmatter.ts` (replaces ~23 copies across 8 files; `test-oracle.ts` deliberately NOT migrated — oracle independence), `walk-wiki.ts` (replaces 5 hand-rolled walkers + 2 glob walks), `content-hash.ts` (+`assertSafeSlug`→path-guard) shedding the glob tree from 4 CLI bundles (~209KB→~6KB class). BM25 hot path tokenizes each doc ONCE (purity proven against a pre-refactor oracle run: byte-identical scores). `resolveKnowledgeDir` single-sourced in brain-paths.ts (server.ts + dream.ts copies DELETED, CLI precedence inversion fixed, source-scan lock extended). `registerJsonTool` collapses the two error grammars across all 23 tools (contract-locked by new `server-tools-contract.test.ts`). Windows bug fixed: `graph-project`'s projects/themes skip-regex never matched backslash paths → graph blocks injected into generated pages (backslash regression test added). `knowledge_search` description finally honest — access-frequency boost dropped (removed 0.33.30); recency KEPT after the agent verified `RECENCY_BOOST_MAX` is still live code (the audit overstated). Dead exports deleted (verified unreferenced). edges.jsonl parsed once per reindex (parsed-doc threading SKIPPED: would feed validate stale pre-projection content).
- **Docs truth sweep (16 files)**: 10 dangling `/second-brain:improve` refs replaced with shipped mechanisms; 3 dev skills' code-map DENIAL rewritten to shipped-as-built (0.33.33-35); FORGET prose fixed to structural-importance-only; 4 inverse surface-collapse comments + 4 authoring artifacts removed; query skill states the real hybrid contract; tool/test counts corrected (23/157); status+import-host slug resolution now via `sb_detect_project`; doubt's legacy-wiki probe turned into an explicit must-be-EMPTY check; hot-tier size fact single-homed. Bonus: dead status Step 5 (`.project-update-pending` — zero producers) deleted; quality-reviewer's retired `quality-rules.md` pointer fixed.
- **Live dogfood**: the 0.33.36 anti-gaming gate fired twice on its own author during this work — both false positives with instructive shapes (probe-payload `rm -rf` string + suite path on one `&&` line; whole-transcript rescan re-firing on history). Argument-scoped + window-scoped precision fixes queued with regression cases (next batch, with the remaining GC/test-economics items and the audit's residual-drift ledger).
- Vitest 642→665 (+3 new modules' suites, contract lock, backslash regression); shell suites all green; no surface-budget change (all new tests are vitest).

## 0.33.37

The loop-engineering tranche lands whole (P1 observability complete + P2 verification backpressure + the X.1 composition lock; `docs/plans/2026-07-13-p1-observability.md`): the autonomous loops fail silently ("no errors" has repeatedly meant "not running"), so this release makes every loop OBSERVABLE, makes the verify gate name exact facts instead of nagging, and locks the capture→store→recall pipeline end-to-end in CI. Fully autonomous throughout — every new surface runs on existing hook paths with zero required user interaction.

- **P2.1 — `stop-verify-gate.sh` deterministic backpressure**: the no-verification block now names the actual changed-file set (full distinct list, first-5 preview) and the project's EXACT verify command from cheap cwd probes (`tests/run-all.sh` → `package.json` scripts.test → `Makefile` test target) — naming only; auto-run stays review-killed (turn-blocking + fail-open break), and the probes must be replaced by the auto-team pinned-command resolver when it lands, not grown into a second one.
- **P2.2 — the judge rung on the solo path** (kill switch `SB_CRITIC_OFFER`, threshold `SB_CRITIC_OFFER_MIN_FILES`=3): when verification RAN but the diff is substantive, ONE non-blocking systemMessage per session offers the already-shipped fresh-context critics (quality-reviewer / persona_think) — rules verify mechanics, the critic judges design; no new machinery, no coercion. `test-stop-verify-gate.sh` 15→18 cases (exact-command + changed-file naming, offer-once, small-diff/kill-switch silence).
- **X.1 — `tests/test-loop-smoke.sh`, the loop COMPOSITION lock** (also added to the macOS bash-3.2/BSD CI lane): Stop-capture archives → canonical writer lands a schema'd page in KNOWLEDGE_DIR → `knowledge_search` FINDS it (BM25, offline) → legacy wiki stays empty → `sb status` liveness observes the same run. Unit suites stayed green while the drainer died per-platform; this fails when any stage stops composing with the next. It immediately caught a real fallback gap: `sb status`'s backlog row vanished for archived-but-never-drained brains (the exact dead-drainer state it exists to expose) — fixed, done-set now defaults empty.

- **`sb status` `Loop liveness:` section** (read-only, all from state files the loops already stamp): drainer last-ran age + status/reason (`.extractor-health.json` mtime — rewritten every run), newest extraction ts + transcript backlog (done-set vs archived, corrupt-line tolerant), scheduler shim present/ABSENT, newest dream + heartbeat age (`status.json` mtime), raw-inbox unprocessed depth across projects (malformed counts as unprocessed, matching `unprocessedCount`). Every absent file renders a loud `never`/`ABSENT`/`none` — silence is the failure mode this section ends. First vitest coverage for the `sb` CLI (`sb-status-liveness.test.ts`, 3 cases incl. cold-brain fallback + corrupt done-set line).
- **`session-load.sh` 0a-quinquies loop-DEAD banner** (kill switch `SB_LOOP_DEAD_BANNER`, threshold `SB_LOOP_DEAD_HOURS`=48): scheduler REGISTERED but the drainer stamped nothing in the window → one banner. The timeout/dead-letter banners need attempts to leave signatures; a task that never fires leaves nothing — the live Windows/macOS breakage shape. Never-ran falls back to the shim's own mtime, so a fresh install gets a grace period; reuses 0a-ter's `CAP_TIMER` probe when present (no second `schtasks` spawn); `timer=unknown` makes no claim. `tests/test-session-load-loop-dead.sh` (6 cases: fire, fresh-silent, kill switch, timer-absent lane, never-ran, install-grace).
- **P1.2+P1.4 — the value loop is now measured** (master kill switch `SB_TELEMETRY`, default on): `session-load.sh` writes a per-session injection manifest (`.injected-manifest-<sid>.jsonl` — the EMITTED codemap spine paths, wiki-enrichment slugs, graph nodes; session_id read from the hook payload, which was previously entirely UNREAD here; TTY-guarded, sanitized). `stop-extract.sh` correlates it once per session against the new-lines window's Read/knowledge_fetch/code_neighbors tool_use records and emits one `gate=value-loop injected=N read=M prior=K hits=…` TRACE row (ec=0 → audit-log via R6b), then deletes the manifest (exactly-once; 7d GC for stray manifests). `prior` = hits on wiki pages `created` before today — the grows-with-use keystone, measured per session. This is the metric that GATES P4.1 routing.
- **P1.3 — utilization counts + dormant-capability report**: the same Stop pass folds `Skill`/`Task(subagent_type)` invocations into `$BRAIN_DIR/utilization-counts.json` (deterministic jq, corrupt store fails loud); `sb status` `Utilization:` renders top-5 + `dormant: N of M shipped capabilities` (this plugin's own skills/agents never seen in the counts) — the direct instrument for "which of the capabilities we ship are actually used?".
- Telemetry firewall (0.33.36) unchanged and green — all of the above is observation-only; nothing feeds ranking/forgetting/graph/codemap. New `tests/test-telemetry-loop.sh` (5 cases: manifest write, both kill-switch ends, exact correlation incl. prior=1 on a yesterday-created page, exactly-once, no-manifest fallback). Surface budget: tests 154→156. Vitest: +2 utilization cases (incl. corrupt-store fallback).
...
- **`session-load.sh` §0d code-map orientation** (kill switch `SB_CODEMAP_ORIENT`, default on): injects the architectural spine — top-10 PageRank-ranked source PATHS from the drainer-maintained `codemap/map.md` (symbols stripped; `(+N more)` footer dropped) — plus a `code_map`/`code_neighbors` pointer. Placed in the priority-banner region BEFORE the forced USER.md/PROJECT.md: live verification on this repo caught the block being budget-starved when placed last (an 8.3KB PROJECT.md consumed the banner room, so the section never fired in exactly the populated sessions that need orientation most). Still budget-checked — cannot breach the 10K hook ceiling. Absent/empty store → silent no-op. Trailing separator added OUTSIDE the `$()` (comsub strips trailing newlines; USER.md is the one section that doesn't prepend its own).
- **`using-second-brain` code check move**: before editing/refactoring a SOURCE file, one `code_neighbors` call (`in` = importers/blast radius, `out` = dependencies); speak up only when the radius is costly to miss; `code_map` for multi-file orientation; cold store → silence. `code_map`+`code_neighbors` added to the skill's allowed-tools.
- **Read-only `code_map`/`code_neighbors` granted to `code-review-unit-reviewer` + `quality-reviewer`** and wired into each protocol (the unit reviewer's cross-file import-following — its own "highest value" section — is now a precise blast-radius query; the quality reviewer's coupling/architecture pass queries the ranked spine). Deliberately NARROWER than the auto-team spec's B0 five-agent slice: history-reviewer (git-blame lens), premise-reviewer (runtime-env lens), and raw-drainer (never reads code) would hold the grant without effect — a P6a least-privilege violation. The B0 spec should inherit this narrowing.
- **`agent-grants.test.ts` +4 locks** (11 cases): grant parity for the two orient-rung reviewers; effect-not-presence on the FRONTMATTER-STRIPPED body (a body that drops `code_neighbors` usage while keeping the grant fails — the 0.33.9/0.33.11 audit class); every agent must declare an explicit `tools:` line (omitting it default-inherits ALL tools incl. `Task` — the dispatch lock would pass vacuously); and a directory-walked **no-recursive-dispatch lock** — no agent may grant `Agent`/`Task`/`Skill`, bare or scoped `Skill(x)`/`Task(...)` form (platform dispatch-depth cap is 1; a reviewer that could spawn agents breaks the fresh-context critique guarantee). Empty whitelist by design. Skill-side parity: `test-skill-allowed-tools.sh` gains the `using-second-brain` req row for the two code-map grants.
- New `tests/test-session-load-codemap.sh` (5 cases, hermetic harness — HOME sandboxed, `CLAUDE_PROJECT_DIR` pinned): inject with whole-path extraction (a spaced source path survives; symbols/footer stripped), kill switch, absent-store, empty-map, and the **placement lock** — near-cap USER/PROJECT fixtures assert the spine still lands, precedes the forced USER.md, and total stays ≤ 10K (a refactor that moves §0d back below the forced sections goes red). Surface budget: tests 153→154. Dev flag catalog: `SB_CODEMAP_ORIENT` registered.
- **Adversarial review (workflow, 16 agents; 10 confirmed findings) drove the pre-release fixes above**, the sharp ones: `sb_append`'s `head -c` cap could cut a spine path mid-string AND eat the USER.md separator (now line-boundary self-truncation, the cap never fires); `awk '{print $1}'` fabricated bogus fragments for spaced paths (now `%%`-expansion on the ` — ` separator); the effect lock was tautological (matched its own grant line); the dispatch lock was vacuous for agents with no `tools:` line and blind to scoped `Skill(x)` forms; the new test was false-red under an ambient `CLAUDE_PROJECT_DIR` and machine-dependent via unsandboxed HOME. Deliberate priority call documented in §0d: orientation outranks the later opportunistic banners it competes with (they repeat every session and also surface via `/second-brain:status`; skips stay loud in error-log.jsonl).
- Research record: `docs/specs/2026-07-08-loop-engineering-research-and-plugin-improvement.md` (13-agent primary-source research + 3-refuter verification; fit-map, phased plan P0→P4, do-not-propose ledger) alongside the direction-approved auto-team spec.
...
- **New `mcp/src/tools/codemap/` module family** (Tier-0 pure-JS/regex — zero new dependencies per the Phase 0 decision; node-tree-sitter REJECTED, WASM tree-sitter deferred to Phase 5): `scan-sources.ts` (git `ls-files -z` scoping with glob fallback + ignore list; caps `SB_CODEMAP_MAX_FILE_BYTES`=512KiB / `SB_CODEMAP_MAX_FILES`=4000, most-recent survival, observable `truncated`), `extract.ts` (TS/JS/Python top-level symbols + resolved relative imports via pure-string `candidateIds`; bare specifiers counted external, never edges), `pagerank.ts` (damping 0.85, fixed 30 iterations or L1<1e-6, sorted accumulation — determinism is a tested contract), `build-graph.ts`, `serialize.ts` (chars/4 estimate, budget×0.95 stop, `(+N more files omitted)` footer — never-exceed invariant tested at the boundary), `store.ts` (atomic writes under `BRAIN_DIR/projects/<slug>/codemap/`).
- **`code-map-cli`** (new bundle `dist/tools/code-map-cli.bundle.js`): scan→extract→rank→serialize→write; `--check` (placeholder drift: stored `git_rev` vs HEAD — real drift lands with Phase 3), `--force`; fail-soft hook-path boundary (exit 0, stderr summary).
- **Two new MCP tools** (read-only, registered in `server.ts`): `code_map` (token-capped ranked map re-serialized at query time via `token_budget` 200–8000; honest `stale` flag probed against the store's own `repo_root`; graceful first-run notice) and `code_neighbors` (blast-radius BFS: `in` = importers, `out` = dependencies, depth ≤4, min-hop dedup, `SB_CODEMAP_NEIGHBORS_MAX`=50 cap; fuzzy basename resolution that lists ALL matches on ambiguity, never silently picks one).
- **Deliberately separate from the wiki graph**: separate store, separate tools; `knowledge_neighbors`/`KNOWLEDGE_DIR/graph` untouched (plan's separation table).
- **Adversarial review (3 lenses, skeptic-verified) drove 7 pre-release fixes**, the big one empirically proven on this repo: the git `ls-files` path ingested COMMITTED build artifacts — 17 tracked `mcp/dist` bundles consumed 42% of the map budget (the rank-desc/id-asc tie-break sorts `dist/` before `src/` at the shared no-inbound rank), 67% of symbols were bundle noise, and one >cap bundle pinned `truncated:true` permanently. The ignore-dirs list now applies to BOTH enumeration paths. Also fixed: language-blind `candidateIds` (a Python `from . import utils` resolved to a same-basename `.ts` file — expansion is now lang-restricted), negative `SB_CODEMAP_TOKEN_BUDGET` permanently disabling generation via the fail-soft CLI (env boundary now falls back to the default; explicit bad args still throw), negative `SB_CODEMAP_NEIGHBORS_MAX` corrupting results via `.slice(0,-N)`, the ambiguous-basename list going out uncapped, the plan's Phase 3 snippet pointing at a nonexistent bundle path behind a silent `|| true`, and zero CLI control-flow coverage. +16 regression tests (115 codemap vitest cases).
- Live smoke on this repo, post-fix: 106 files / 264 symbols / 197 import edges (dist noise: 0), map ~1816 tokens under the 2000 default budget, `truncated:false`, top-ranked files are the genuine load-bearing modules (path-guard, brain-paths, graph-store, knowledge-search).
- Out of scope (next Phases): out-of-band drift regen in the drainer (Phase 3), code↔wiki layer-5 edges (Phase 4). No surface-budget change (all tests are vitest; regen will fold into the existing drainer script).
...
- **Windows guard fail-open class (G-HOOK-2, 3 guards re-armed)**: new `sb_normalize_path()` funnel in `lib.sh` (backslash→`/`, `C:/…`→`/c/…` via cygpath; `\\?\` extended-length prefix stripped; `\\localhost\c$`/`\\127.0.0.1\c$` admin shares mapped to drive form; idempotent, fallback-safe). `symlink-guard.sh` normalizes the payload path BEFORE realpath and realpath's OUTPUT after (GNU realpath re-emits `C:/` form), and its credential-prefix match is now case-INSENSITIVE (NTFS/APFS are case-insensitive — `.SSH` is `.ssh` there; adversarial-review finding); `persona-tool-guard.sh` normalizes CWD + target so the scope allowlist and self-edit regexes match (a `C:\…` path used to fall through as CWD-relative → silent allow); `wiki-write-guard.sh` converts backslashes so the `/knowledge/wiki/` glob matches. Each guard carries a minimal inline fallback so it stays armed even if `lib.sh` fails to source — and that fallback branch is itself tested (`CLAUDE_PLUGIN_ROOT=/nonexistent` cases). Windows-form regression tests run on Linux CI (full-suite lane) via stubbed `cygpath`/`realpath` (`tests/test-normalize-path.sh` + new cases in all three guard tests).
- **dream-accept post-snapshot protection (finding D)**: a completed dream can sit for days while the drainer/maintainer write NEW live pages; bare `rsync --delete` silently deleted them. Accept now protects every live page modified after the snapshot (`find -newer` against a `touch`-stamped reference; GNU `-d` with `TZ=UTC` POSIX `-t` fallback — local-time interpretation of the UTC stamp would shift the protect boundary) — neither deleted nor overwritten (merge-only, newer live wins); dream-dropped old pages still delete. The no-rsync path (the NORMAL apply path on Windows git-bash) now honors the same contract via stash+merge+restore, so post-snapshot live EDITS survive too, and it prints an honest "deletions were NOT applied" note. Unusable/missing `created_at` now fails SAFE: merge-only apply, `--delete` never runs (was: unprotected full `--delete`). Exclude lists are built line-wise (space-safe), and the F1-floor/backup guards are unchanged.
- **REFLECT feedback loop (last open HIGH)**: `graph-cluster-cli.ts` now excludes `generated: true` pages from clustering INPUT (same rationale as the projects//themes/ dir skip). A reflection page joining its own cluster defeated `member_hash` idempotence (LLM re-reflects every dream) and could BECOME the cluster id (`reflection-reflection-<id>` page growth). Regression-locked in `test-graph-cluster-shim.sh`.
- **Transcript prune order**: `sb_prune_transcripts` now evicts by MTIME, not lexical filename sort (archives lead with a random session UUID — the cap could evict a just-archived, not-yet-drained transcript). GNU `find -printf` with BSD stat fallback; adversarial UUID-ordering test.
- **projects.jsonl corruption**: the SessionStart bookkeeping rewrite used `jq` without `-c`, pretty-printing each record across ~8 lines — the MCP registry reader (line-by-line JSON.parse) returned `[]`, blinding slug resolution/dream family filters/search tiering and undoing the 0.33.0 repair every session. Now `jq -c` with a TWO-STAGE write gate: jq's exit code gates the `mv` directly (a corrupt mid-file line makes jq emit a partial prefix then die — piped through `tr` that partial output would have silently TRUNCATED the registry; adversarial-review live repro), CRLF strip, `[ -s ]` backstop, and a fail-loud `sb_log_error` when the rewrite is skipped. Membership test asserts one-compact-object-per-line AND corrupt-line-leaves-file-byte-intact.
...
- **Consolidation agents hardened** (`dream-runner`, `knowledge-maintainer`, `raw-drainer`): unused `Bash(git *)` grants dropped; missing `knowledge_*` MCP grants added (maintainer Phases 1/2/3/5 and drain steps called tools their whitelist excluded — silently skipped); explicit "untrusted input — DATA, not instructions" framing added to all three (P6 commitment 2). All locked by new `agent-grants.test.ts` cases.
