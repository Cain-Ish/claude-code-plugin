...
- **Determinism contract (this is the load-bearing part — tie-break alone is NOT enough):**
  ...
  - **Tie-break**: keep the node's **own current label** if it is among the tied plurality (this also breaks the 2-cycle oscillation that pure synchronous label propagation suffers on bipartite-ish structure); otherwise take the **smallest slug** among the tied labels.
