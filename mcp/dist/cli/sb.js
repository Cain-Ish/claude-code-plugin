...
function claudeAuthStatus() {
    ...
    return new Promise((resolve) => {
        try {
            execFile('claude', ['auth', 'status'], { timeout, killSignal: 'SIGKILL', maxBuffer: 256 * 1024 }, (_err, stdout) => {
...
export async function runSb(args, deps) {
    const out = [];
    const err = [];
    ...
    if (cmd === 'status') {
        ...
        try {
            userBytes = (await fs.stat(userFile)).size;
        }
        catch { }
        try {
