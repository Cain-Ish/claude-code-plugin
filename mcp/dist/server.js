...
const server = new McpServer({ name: "knowledge-base", version: "2.3.1" }, {
...
server.registerTool("episodic_search", {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `Episodic search error: ${error instanceof Error ? error.message : String(error)}` }],
            isError: true,
        };
    }
});
server.registerTool("episodic_read", {
...
server.registerTool("knowledge_relate", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Relate error: ${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
});
server.registerTool("knowledge_neighbors", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Neighbors error: ${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
});
// --- Start ---
async function main() {
