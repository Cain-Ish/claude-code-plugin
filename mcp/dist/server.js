...
const server = new McpServer({ name: "knowledge-base", version: "0.2.0" }, {
...
async function indexFile(vectordb, filePath, force = false) {
    let stat;
    try {
...
async function maybeAutoIndex(vectordb) {
    const now = Date.now();
    if (now - lastAutoIndexCheck < AUTO_INDEX_COOLDOWN)
        return;
    ...
    try {
        ...
        const formatted = results.map((r, i) => {
            ...
            if (full) {
                let body = r.excerpt;
                try {
                    body = fs.readFileSync(r.path, "utf-8");
                }
                catch { /* fall back to excerpt */ }
...
async function multiConceptSearch(vectordb, concepts, limit, category) {
    const widerLimit = limit * 3;
    const lowerMinScore = 0.2;
    const resultSets = await Promise.all(concepts.map(async (concept) => {
        const embedding = await embed(concept);
        return vectordb.search(embedding, widerLimit, category, lowerMinScore, concept);
    }));
    const scoreMaps = resultSets.map((results) => new Map(results.map((r) => [r.path, r])));
...
server.registerTool("knowledge_feedback", {
    ...
    try {
        const rawResolved = path.resolve(filePath.startsWith("~") ? path.join(os.homedir(), filePath.slice(1)) : filePath);
        let resolved;
        try {
        ...
        let knowledgeResolved, brainResolved;
        try {
            knowledgeResolved = fs.realpathSync(path.resolve(KNOWLEDGE_DIR));
        }
        catch {
            knowledgeResolved = path.resolve(KNOWLEDGE_DIR);
        }
        try {
            brainResolved = fs.realpathSync(path.resolve(path.join(os.homedir(), ".second-brain")));
        }
        catch {
            brainResolved = path.resolve(path.join(os.homedir(), ".second-brain"));
        }
        if (!resolved.startsWith(knowledgeResolved + path.sep) && !resolved.startsWith(brainResolved + path.sep)
            ...
            return {
                content: [{ type: "text", text: "Path must be within knowledge or second-brain directory" }],
                isError: true,
            };
        }
        ...
        if (!updated) {
            return {
                content: [{ type: "text", text: `Page not found in index: ${resolved}` }],
                isError: true,
            };
        }
        return {
            content: [{ type: "text", text: `Feedback recorded for ${path.basename(resolved)}: ${helpful ? "helpful (+1)" : "not helpful (-1)"}${context ? ` — ${context}` : ""}` }],
        };
    }
    catch (error) {
        return {
            content: [{ type: "text", text: `Feedback error: ${error instanceof Error ? error.message : String(error)}` }],
            isError: true,
        };
    }
});
function updateLearningFeedback(resolvedPath, helpful, context, entry) {
