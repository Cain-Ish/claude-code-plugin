...
function fakeEmbedding(seed, dims = 64) {
...
describe("VectorDB", () => {
    ...
    it("upsertPage + isIndexed round-trip", () => {
        const now = Date.now();
        db.upsertPage({ path: "wiki/test.md", title: "Test", content: "hello world", category: "wiki", lastModified: now }, fakeEmbedding(1));
    ...
    it("search returns results sorted by score", () => {
        db.upsertPage({ path: "wiki/a.md", title: "Alpha", content: "alpha content", category: "wiki", lastModified: Date.now() }, fakeEmbedding(10));
        db.upsertPage({ path: "wiki/b.md", title: "Beta", content: "beta content", category: "wiki", lastModified: Date.now() }, fakeEmbedding(99));
    ...
    it("search filters by category", () => {
        db.upsertPage({ path: "sessions/s.md", title: "S", content: "session", category: "sessions", lastModified: Date.now() }, fakeEmbedding(50));
    ...
    it("search with queryText uses BM25 hybrid scoring", () => {
        db.upsertPage({ path: "wiki/auth.md", title: "Authentication", content: "JWT tokens OAuth authentication flow", category: "wiki", lastModified: Date.now() }, fakeEmbedding(5));
    ...
    it("recordAccess increments and updateAccessCount works", () => {
        db.upsertPage({ path: "wiki/hot.md", title: "Hot", content: "popular page", category: "wiki", lastModified: Date.now() }, fakeEmbedding(3));
    ...
    it("access-count boost: higher count ranks higher with equal similarity", async () => {
        ...
        boostDb.upsertPage({ path: "wiki/popular.md", title: "Popular", content: "xyz unique content popular", category: "wiki", lastModified: Date.now() }, sharedEmb);
        boostDb.upsertPage({ path: "wiki/unpopular.md", title: "Unpopular", content: "xyz unique content unpopular", category: "wiki", lastModified: Date.now() }, sharedEmb);
        for (let i = 0; i < 10; i++)
            boostDb.recordAccess(["wiki/popular.md"]);
    ...
    it("removeStale removes unlisted paths", () => {
        db.upsertPage({ path: "wiki/keep-me.md", title: "Keep", content: "keep", category: "wiki", lastModified: Date.now() }, fakeEmbedding(11));
        db.upsertPage({ path: "wiki/remove-me.md", title: "Remove", content: "remove", category: "wiki", lastModified: Date.now() }, fakeEmbedding(12));
        const existing = new Set();
        const all = db.search(fakeEmbedding(1), 100, undefined, 0.0);
        for (const r of all) {
            if (r.path !== "wiki/remove-me.md")
                existing.add(r.path);
        }
    ...
    it("rejects corrupted embedding blobs", async () => {
        ...
        await db3.ready();
        db3.upsertPage({ path: "wiki/good.md", title: "Good", content: "valid page", category: "wiki", lastModified: Date.now() }, fakeEmbedding(70));
        db3.close();
        ...
        rawDb.run("INSERT OR REPLACE INTO pages (path, title, content, category, lastModified, indexedAt, embedding, accessCount, lastAccessed) VALUES (?, ?, ?, ?, ?, ?, ?, 0, NULL)", ["wiki/corrupt.md", "Corrupt", "corrupt page", "wiki", Date.now(), Date.now(), new Uint8Array([1, 2, 3, 4, 5])]);
    ...
    it("reopens existing SQLite database", async () => {
        ...
        await db1.ready();
        db1.upsertPage({ path: "wiki/persist.md", title: "Persist", content: "test", category: "wiki", lastModified: Date.now() }, fakeEmbedding(60));
        db1.close();
