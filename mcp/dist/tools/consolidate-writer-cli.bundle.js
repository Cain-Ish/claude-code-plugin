...
var AI_BLOCK_RE, AI_BLOCK_SCHEMAS, AI_BLOCK_RE_G;
var init_ai_block = __esm({
  "src/tools/ai-block.ts"() {
    "use strict";
    init_kb_schema2();
    AI_BLOCK_RE = /<!--\s*ai:begin[^\n]*?-->\n?([\s\S]*?)<!--\s*ai:end\s*-->/;
    AI_BLOCK_SCHEMAS = AI_BLOCK_TYPES;
...
`;
    ...
    try {
      validateSlug(slug);
      let path2 = assertWithin(wikiRoot, category, `${slug}.md`);
      let existing = null;
      try {
        existing = await fs.readFile(path2, "utf-8");
      } catch {
      }
      if (existing !== null) {
        ...
        slug = `${slug}-${hash}`;
        validateSlug(slug);
        path2 = assertWithin(wikiRoot, category, `${slug}.md`);
        try {
          const clash = await fs.readFile(path2, "utf-8");
      ...
      await writeAtomic(path2, renderNewPage(f, category, title, hash, opts));
...
// src/brain-paths.ts
import { join as join2, isAbsolute as isAbsolute3 } from "path";
...
function resolveKnowledgeDir(override) {
  ...
  for (const raw of [process.env.CLAUDE_PLUGIN_OPTION_KNOWLEDGE_DIR, process.env.KNOWLEDGE_DIR]) {
    ...
    return c.startsWith("~") ? join2(homedir(), c.slice(1)) : c;
  }
  return join2(homedir(), "knowledge");
}
var init_brain_paths = __esm({
  "src/brain-paths.ts"() {
    "use strict";
    init_path_guard();
  }
...
async function logLoadError(message, brainDir) {
  ...
  try {
    await fs3.mkdir(brainDir, { recursive: true });
    await fs3.appendFile(join3(brainDir, "error-log.jsonl"), JSON.stringify(entry) + "\n");
  } catch {
  }
  try {
...
async function loadCache(wikiRoot) {
  try {
    const data = await fs3.readFile(join3(wikiRoot, CACHE_FILE), "utf-8");
...
async function saveCache(wikiRoot, cache) {
  await atomicWriteJson(join3(wikiRoot, CACHE_FILE), cache);
}
...
var EMBEDDING_DIM, CACHE_FILE, MODEL_ID, DISABLE_ENV, pipelineInstance, lastLoadError;
var init_embeddings = __esm({
  "src/tools/embeddings.ts"() {
    ...
    DISABLE_ENV = "SECOND_BRAIN_DISABLE_EMBEDDINGS";
...
var S, W, D, G, M, C, P, H, F, U, O, R, L;
var init_index_min = __esm({
  "node_modules/lru-cache/dist/esm/node/index.min.js"() {
    S = I("lru-cache:metrics");
    W = j("lru-cache");
    D = () => S.hasSubscribers || W.hasSubscribers;
    ...
    H = (u3) => !M.has(u3);
    F = (u3) => !!u3 && u3 === Math.floor(u3) && u3 > 0 && isFinite(u3);
    U = (u3) => F(u3) ? u3 <= Math.pow(2, 8) ? Uint8Array : u3 <= Math.pow(2, 16) ? Uint16Array : u3 <= Math.pow(2, 32) ? Uint32Array : u3 <= Number.MAX_SAFE_INTEGER ? O : null : null;
...
var init_esm5 = __esm({
  "node_modules/path-scurry/dist/esm/index.js"() {
    ...
    normalizeNocaseCache = new L({ max: 2 ** 12 });
    normalizeNocase = (s) => {
    ...
    ResolveCache = class extends L {
      constructor() {
        super({ max: 256 });
      }
    };
    ChildrenCache = class extends L {
...
import { join as join4, relative as relative2, resolve as resolve3, sep as sep3, isAbsolute as isAbsolute4 } from "path";
function registryPath(brainDir, slug) {
  return join4(brainDir, "projects", slug, "doc-sources.json");
}
async function loadRegistry(brainDir, slug) {
  try {
    assertSafeSlug(slug);
    return JSON.parse(await fs4.readFile(registryPath(brainDir, slug), "utf-8"));
  } catch {
    return null;
  }
}
var init_doc_sources = __esm({
  "src/tools/doc-sources.ts"() {
    "use strict";
    init_esm6();
    init_path_guard();
...
function isValidRecord(r) {
  ...
  if (!EDGE_TYPES2.includes(r.type)) return false;
...
async function loadEdges(path2) {
  let raw;
  try {
    raw = await fs5.readFile(path2, "utf-8");
  } catch {
    return [];
  }
  const out = [];
...
var init_graph_store = __esm({
  "src/tools/graph-store.ts"() {
    "use strict";
    EDGE_TYPES2 = ["requires", "affects", "relates", "part_of", "supersedes"];
  }
});
...
function loadRegistry2(brainDir) {
  let text;
  try {
    text = readFileSync(join5(brainDir, "projects.jsonl"), "utf-8");
  } catch {
    return [];
  }
  const out = [];
...
function projectFamily(brainDir, slug) {
  const recs = loadRegistry2(brainDir);
...
__export(knowledge_search_exports, {
  ...
  extractYamlValue: () => extractYamlValue,
...
async function loadAccessCounts() {
  try {
    return JSON.parse(await fs7.readFile(accessCountsFile(), "utf-8"));
  } catch {
    return {};
  }
}
...
var ACCESS_PRUNE_DAYS, TOP_K, SNIPPET_CHARS, BM25_K1, BM25_B, AVG_DOC_LENGTH, DATE_TOKEN_RE, MIN_SCORE_RATIO, STUB_PENALTY, MIN_SUBSTANTIVE_LENGTH, AUTO_EXTRACTED_RE;
var init_knowledge_search = __esm({
  "src/tools/knowledge-search.ts"() {
    ...
    init_walk_wiki();
    init_frontmatter();
    ACCESS_PRUNE_DAYS = 90;
    TOP_K = 8;
    SNIPPET_CHARS = 200;
    BM25_K1 = 1.2;
    BM25_B = 0.75;
    AVG_DOC_LENGTH = 200;
    DATE_TOKEN_RE = /^\d{4}$|^\d{2}$/;
    MIN_SCORE_RATIO = 0.15;
    STUB_PENALTY = 0.5;
    MIN_SUBSTANTIVE_LENGTH = 100;
    AUTO_EXTRACTED_RE = /<!--\s*auto-extracted/;
  }
});
...
async function main() {
  ...
  process.env.SB_BRAIN_DIR = join8(dreamDir, ".cw-scratch");
  await fs8.mkdir(process.env.SB_BRAIN_DIR, { recursive: true });
  ...
  const { knowledgeSearch: knowledgeSearch2 } = await Promise.resolve().then(() => (init_knowledge_search(), knowledge_search_exports));
  const stagingRoot = join8(dreamDir, "staging");
  try {
    await fs8.access(join8(stagingRoot, "wiki"));
  } catch {
...
`);
  ...
  let rawText;
  try {
    rawText = await fs8.readFile(join8(dreamDir, "candidate-facts.json"), "utf-8");
  } catch {
...
`);
  ...
  if (!date) {
    try {
      const st = JSON.parse(await fs8.readFile(join8(dreamDir, "status.json"), "utf-8"));
...
`);
  ...
  const report = await applyCandidates2(stagingRoot, facts, {
    dreamId: base,
    date,
    searchFn: async (query, scope) => {
      const r = await knowledgeSearch2({ query, scope, knowledgeDir: stagingRoot });
