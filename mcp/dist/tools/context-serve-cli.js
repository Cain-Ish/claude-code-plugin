...
try {
    if (!brainDir)
        throw new Error('no brain dir resolvable');
    const result = await episodicSearch({ query, limit: 2, mode: 'both', activeProject: projectSlug }, brainDir);
...
for (const l of wikiLines)
    console.log(l);
console.log(SEP);
for (const l of epiLines)
    console.log(l);
//# sourceMappingURL=context-serve-cli.js.map