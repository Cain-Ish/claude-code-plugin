...
async function scanLocations(projectRoot, locations) {
  ...
  const within = (p) => {
    const r = resolve(p);
    return r === rootResolved || r.startsWith(rootResolved + sep2);
  };
  for (const loc of locations) {
    ...
    for (const m of matches) if (within(m) && !seen.has(m)) {
      seen.add(m);
      absPaths.push(m);
    }
  }
  ...
  for (const p of kept) {
    try {
      ...
      entries.push({
        id: hash.slice(0, 12),
        path: p,
        rel: relative(projectRoot, p).split(/[\\/]+/).join("/"),
...
async function main() {
  ...
  if (!slug) {
    process.stderr.write("doc-sources-cli: missing slug arg\n");
    return;
  }
  try {
