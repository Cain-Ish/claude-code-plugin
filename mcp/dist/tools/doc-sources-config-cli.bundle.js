...
async function readConfig(brainDir, slug) {
  try {
    ...
    return { locations: Array.isArray(j2.locations) ? j2.locations : [] };
  } catch {
    return { locations: [] };
  }
}
...
async function listLocations(brainDir, slug) {
  assertSafeSlug(slug);
...
async function removeLocation(brainDir, slug, location) {
  assertSafeSlug(slug);
  const loc = normalizeLocation(location);
  const cfg = await readConfig(brainDir, slug);
...
async function main() {
  ...
  if (!slug) {
    console.log("track: could not resolve the active project (no slug)");
    return;
  }
  try {
    if (action === "list") {
      ...
      if (!location) {
        console.log("usage: track add <path-or-glob>");
        return;
      }
      ...
      if (!location) {
        console.log("usage: track remove <path-or-glob>");
        return;
      }
