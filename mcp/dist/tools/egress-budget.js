...
export function egressBudgetTokens() {
 ...
 */
export function capText(text, maxTokens, pointer) {
    ...
    for (const { segment } of segmenter.segment(text)) {
        if (out.length + segment.length > keepChars)
            break;
        out += segment;
    }
 ...
 */
export function capList(items, render, maxTokens, moreHint, separator = '\n\n') {
    const kept = [];
    const parts = [];
    let used = 0;
...
//# sourceMappingURL=egress-budget.js.map