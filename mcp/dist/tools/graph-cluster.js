...
export function buildAdjacency(pages) {
...
export function labelPropagate(adj, opts = {}) {
 ...
 *  labelled neighbours. */
export function assignNewNode(adj, labels, node) {
    const nbrs = adj.get(node);
    if (!nbrs || nbrs.size === 0)
        return node;
    const tally = new Map();
...
export function clusters(labels, opts) {
    const groups = new Map();
    for (const [node, lab] of labels) {
        let g = groups.get(lab);
        if (!g) {
    ...
    for (const [id, members] of groups) {
        if (members.length >= opts.minSize)
            out.push({ id, members: [...members].sort() });
    }
...
export function memberHash(sortedMembers, contentHashBySlug) {
    const basis = sortedMembers.join('|') + '::' +
        sortedMembers.map(s => contentHashBySlug[s] ?? '').join('|');
    return djb2(basis);
}
//# sourceMappingURL=graph-cluster.js.map