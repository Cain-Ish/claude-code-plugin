...
import { buildAdjacency, labelPropagate, assignNewNode, clusters, memberHash, } from './graph-cluster.js';
...
const norm = (m) => [...m.entries()].sort().map(([k, v]) => `${k}:${v}`).join(',');
...
describe('labelPropagate (deterministic)', () => {
    ...
    it('is byte-identical across repeated runs AND shuffled insertion order', () => {
        ...
        const shuffled = adjOf([...barbell].reverse().map(([u, v]) => [v, u]));
...
//# sourceMappingURL=graph-cluster.test.js.map