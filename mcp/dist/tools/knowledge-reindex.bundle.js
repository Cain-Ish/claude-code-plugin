...
async function knowledgeValidate(knowledgeDir, opts = {}) {
  ...
  const issues = [];
  let fixed = 0;
  const edgeRecords = opts.edges ?? await loadEdges(join2(knowledgeDir, "graph", "edges.jsonl"));
  ...
  try {
    const rootFiles = await fs3.readdir(knowledgeDir, { withFileTypes: true });
...
async function patchFrontmatter(filePath, wikiDir, graphEnabled = false) {
  ...
  let mtimeDate;
  try {
    mtimeDate = (await fs3.stat(filePath)).mtime.toISOString().slice(0, 10);
  } catch {
...
`;
  await fs3.writeFile(filePath, fm + original, "utf-8");
}
...
async function projectGraphToPages(knowledgeDir, preloadedEdges) {
  const records = preloadedEdges ?? await loadEdges(join3(knowledgeDir, "graph", "edges.jsonl"));
  ...
  await Promise.all(files.map(async (f) => {
    try {
      const head = (await fs4.readFile(f, "utf-8")).slice(0, 4096);
...
`);
    ...
    if (content !== before) {
      await fs4.writeFile(file, content, "utf-8");
      updated++;
    }
  }
...
async function knowledgeReindex(knowledgeDir) {
  const edgeRecords = await loadEdges(join4(knowledgeDir, "graph", "edges.jsonl"));
  try {
  ...
  if (mocs.size > 0) await fs5.mkdir(projDir, { recursive: true });
  for (const existing of await mocSlugs(projDir)) {
    if (!mocs.has(existing)) {
      try {
        await fs5.unlink(join4(projDir, `${existing}.md`));
      } catch {
      }
    }
  }
  for (const [proj, region] of mocs) {
    ...
    await fs5.writeFile(join4(projDir, `${proj}.md`), header + region + "\n", "utf-8");
  }
  const sections = [
    "---",
  ...
  await fs5.writeFile(indexPath, sections.join("\n"), "utf-8");
...
async function mocSlugs(dir) {
  try {
    return (await fs5.readdir(dir)).filter((f) => f.endsWith(".md") && f !== "index.md").map((f) => f.replace(/\.md$/, "")).sort();
  } catch {
    return [];
  }
}
