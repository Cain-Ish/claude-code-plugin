...
export function slugFromProjectDir(dir) {
    ...
    if (!base || base === '/' || base === '.' || base === '..')
        return undefined;
    if (/^tmp\.|^tmp$|^\.tmp\.|^tmpfs$/.test(base))
        return 'scratch';
    return base;
}
...
export function resolveActiveSlug(brainDir, env = process.env, cwd = process.cwd) {
    ...
    if (cwdSlug && existsSync(join(brainDir, 'projects', cwdSlug, 'PROJECT.md')))
        return cwdSlug;
    try {
