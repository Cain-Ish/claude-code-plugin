...
/** The settings.json command: chained statusline (if any) as env, then the shim — both single-quoted. */
export function statuslineCommand(brainDir: string, chain: string | null): string {
  const env = chain ? `SB_BUDDY_CHAIN=${shq(chain)} ` : '';
  return `${env}bash ${shq(posix(shimPath(brainDir)))}`;
}

/** Inverse of statuslineCommand, for this and the 0.51.0 form (`bash "<shim>"`). null = not ours. */
export function parseOurCommand(command: string): { chain: string | null } | null {
  ...
  return { chain: m[1] !== undefined ? unshq(m[1]) : null };
}
...
async function readSettings(file: string): Promise<{ raw: string; settings: Obj }> {
  let raw = '';
  try { raw = await fs.readFile(file, 'utf-8'); } catch { return { raw: '', settings: {} }; }
...
async function writeSettings(file: string, settings: Obj): Promise<void> {
 ...
 *  other fields (padding …) kept. Idempotent; an older buddy install (no refreshInterval, the
 *  0.51.0 quoting, a stale shim) is upgraded in place, a user-set refreshInterval respected.
 *  Sets buddy.json react:true — installing is the consent for the two-way [buddy] line. */
export async function installStatusline(brainDir: string, pluginRoot: string): Promise<InstallResult> {
  ...
  await patchConfig(brainDir, { react: true, chain: null, identity: null });   // `chain`: 0.51.0 record, never read again
  ...
  if (!parsed) await patchConfig(brainDir, { prev_statusline: prev ?? null });   // for uninstall: padding etc. of the chained line
 ...
 *  back, with the fields recorded at install only when that record's command matches it. */
