...
function loadRegistry(brainDir) {
  let text;
  try {
    text = readFileSync2(join2(brainDir, "projects.jsonl"), "utf-8");
  } catch {
    return [];
  }
  const out = [];
...
function resolveActiveSlug(brainDir, env = process.env, cwd = process.cwd) {
  ...
  if (cwdSlug && existsSync2(join3(brainDir, "projects", cwdSlug, "PROJECT.md"))) return cwdSlug;
  try {
    const pin = readFileSync3(join3(brainDir, ".active-session-slug"), "utf-8").trim();
    if (pin && existsSync2(join3(brainDir, "projects", pin, "PROJECT.md"))) return pin;
  } catch {
  }
  return cwdSlug;
}
...
async function scanSources(repoRoot, opts = {}) {
  ...
  try {
    ...
    candidates = stdout.split("\0").filter((rel) => rel.length > 0).map((rel) => ({ id: rel, abs: path2.resolve(repoRoot, rel) }));
  } catch {
    ...
    candidates = absList.map((abs) => ({
      ...
      id: path2.relative(repoRoot, abs).split(path2.sep).join("/"),
      abs
    }));
  }
  ...
  for (const { id, abs } of candidates) {
    ...
    const lang = EXT_LANG[path2.extname(id).toLowerCase()];
...
var KNOWN_EXTS = [".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".py"];
var INDEX_EXTS = [".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"];
var ESM_SOURCE_ALIAS = { ".js": ".ts", ".jsx": ".tsx" };
...
function expandBase(base, lang) {
  ...
  const ext2 = extMatch ? extMatch[0] : "";
  if (ext2 && KNOWN_EXTS.includes(ext2)) {
    const out2 = [base];
    const alias = ESM_SOURCE_ALIAS[ext2];
    if (alias) out2.push(base.slice(0, -ext2.length) + alias);
    return out2;
  }
  if (ext2) return [base];
  const out = [];
...
var RE_EXPORT_FN = /^export\s+(?:async\s+)?function\s*\*?\s*([A-Za-z_$][\w$]*)/gm;
var RE_EXPORT_CLASS = /^export\s+(?:abstract\s+)?class\s+([A-Za-z_$][\w$]*)/gm;
var RE_EXPORT_VAR = /^export\s+(?:const|let|var)\s+([A-Za-z_$][\w$]*)/gm;
var RE_EXPORT_BRACE = /^export\s+(?:type\s+)?\{([^}]*)\}/gm;
var RE_EXPORT_DEFAULT = /^export\s+default\b(?:\s+(?:async\s+)?(?:function\s*\*?|class)\s+([A-Za-z_$][\w$]*))?/m;
var RE_PLAIN_FN = /^(?:async\s+)?function\s*\*?\s*([A-Za-z_$][\w$]*)/gm;
var RE_PLAIN_CLASS = /^(?:abstract\s+)?class\s+([A-Za-z_$][\w$]*)/gm;
var RE_FROM = /\bfrom\s*(['"])([^'"\n]+)\1/g;
var RE_SIDE_EFFECT_IMPORT = /^\s*import\s*(['"])([^'"\n]+)\1/gm;
var RE_REQUIRE = /\brequire\s*\(\s*(['"])([^'"\n]+)\1\s*\)/g;
var RE_DYNAMIC_IMPORT = /\bimport\s*\(\s*(['"])([^'"\n]+)\1\s*\)/g;
function parseTsJs(src) {
  const symbols = [];
...
function parseBraceNames(inner) {
  const names = [];
...
var RE_PY_DEF = /^(?:async\s+)?def\s+([A-Za-z_]\w*)/gm;
var RE_PY_CLASS = /^class\s+([A-Za-z_]\w*)/gm;
var RE_PY_IMPORT = /^\s*import\s+(.+)$/gm;
var RE_PY_FROM = /^\s*from\s+([.\w]+)\s+import\s+(.+)$/gm;
...
// src/tools/codemap/build-graph.ts
...
function codemapDir(brainDir, slug) {
  return path3.join(brainDir, "projects", slug, "codemap");
}
...
async function writeGraph(dir, graph, mapMd) {
  await fs.mkdir(dir, { recursive: true });
  await atomicWrite(path3.join(dir, "graph.json"), JSON.stringify(graph) + "\n");
  await atomicWrite(path3.join(dir, "map.md"), mapMd);
}
async function readGraph(dir) {
  const file = path3.join(dir, "graph.json");
  let raw;
  try {
