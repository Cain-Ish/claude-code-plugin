...
if [ -n "$PENDING_ID" ]; then
  ...
  sb_buddy_event _global pending waiting "Dream ${PENDING_ID} is staged but never ran — /second-brain:dream resumes it." dream-autostage 0
  exit 0
fi
...
sb_buddy_event _global pending waiting "${NEW} new transcripts since the last dream — /second-brain:dream consolidates them into the wiki." dream-autostage 0
exit 0
