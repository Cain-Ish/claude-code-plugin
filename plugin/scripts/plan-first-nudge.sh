...
if [ "$SPINE_ON" = "1" ] && [ -f "$STATE/$SID.gate" ] && [ ! -f "$DONE" ]; then
  ...
  command -v sb_buddy_event >/dev/null 2>&1 && sb_buddy_event "$SID" gate pleased "Plan recorded — gate cleared ($(grep -c . "$SEEN" 2>/dev/null || true) file(s) in scope)." plan-first-nudge 120
  exit 0
fi
...
if [ "$SPINE_ON" = "1" ] && [ -f "$STATE/$SID.driftdeny" ]; then
  ...
  if [ -n "$_SQ" ]; then
    ...
    command -v sb_buddy_event >/dev/null 2>&1 && sb_buddy_event "$SID" gate pleased "Scope change confirmed — re-ground gate cleared." plan-first-nudge 120
  fi
fi
      ...
      if [ "$_OBSERVED" -eq 0 ]; then
        ...
        if [ "${DCOUNT:-0}" -ge "$DTHRESH" ] 2>/dev/null; then
          ...
          if [ ! -f "$STATE/$SID.driftwarn" ]; then
            ...
            command -v sb_buddy_event >/dev/null 2>&1 && sb_buddy_event "$SID" gate puzzled "Drifting? ${DCOUNT} files share no keyword with the goal. Say the scope changed, or return to it." plan-first-nudge
            jq -nc --arg ctx "$CTX" '{
              hookSpecificOutput: {
                hookEventName: "PreToolUse",
                additionalContext: $ctx
          ...
          command -v sb_buddy_event >/dev/null 2>&1 && sb_buddy_event "$SID" gate alert "Re-ground gate: second off-goal file set — confirm the scope change or return to goal, then retry." plan-first-nudge
          jq -nc --arg r "$REASON" '{
            hookSpecificOutput: {
              hookEventName: "PreToolUse",
              permissionDecision: "deny",
