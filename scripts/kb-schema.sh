...
if command -v jq >/dev/null 2>&1 && [ -f "$_SB_KB_SCHEMA" ]; then
  SB_STRUCTURED_TYPES=$(jq -r '.structured_types | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_UNSTRUCTURED_TYPES=$(jq -r '.unstructured_types | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_GENERATED_DIRS=$(jq -r '.generated_dirs | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_CONTENT_CATEGORIES=$(jq -r '(.structured_types + .unstructured_types) | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_ALL_CATEGORIES=$(jq -r '(.structured_types + .unstructured_types + .generated_dirs) | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_EDGE_TYPES=$(jq -r '.edge_types | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_FORGET_PROTECTED=$(jq -r '.forget_protection.protected | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_FORGET_DISCOUNTED=$(jq -r '.forget_protection.discounted | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_RAW_DIR=$(jq -r '.raw.dir' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
  SB_RAW_STATUSES=$(jq -r '.raw.statuses | join(" ")' "$_SB_KB_SCHEMA" 2>/dev/null | tr -d '\r')
