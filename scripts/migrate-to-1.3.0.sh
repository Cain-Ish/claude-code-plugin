...
if command -v node >/dev/null 2>&1 && [ -f "$PLUGIN_ROOT/mcp/dist/tools/knowledge-reindex.bundle.js" ]; then
  SB_BUNDLE="$PLUGIN_ROOT/mcp/dist/tools/knowledge-reindex.bundle.js" SB_KDIR="$KNOWLEDGE_DIR" node -e "
    import { knowledgeReindex } from process.env.SB_BUNDLE;
    knowledgeReindex(process.env.SB_KDIR).then(r => console.log('  [7/8] Index generated: ' + r.pagesIndexed + ' pages.'))
