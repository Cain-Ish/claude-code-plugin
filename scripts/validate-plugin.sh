...
if [ -f "$HOOKS" ]; then
  if ! jq empty "$HOOKS" 2>/dev/null; then
    ...
    # An unquoted ${CLAUDE_PLUGIN_ROOT} word-splits when the install path has a space
    # (C:\Users\John Smith\…): every hook dies with exit 127. `claude plugin validate --strict`
    # flags it (CLI 2.1.281+), but CI has no claude CLI — this static check is the CI lock.
    UNQ=$(jq -r '.. | .command? // empty' "$HOOKS" 2>/dev/null | tr -d '\r' | grep -E '(^|[^"])\$\{CLAUDE_PLUGIN_ROOT\}' || true)
    if [ -n "$UNQ" ]; then
      echo "FAIL: hooks.json command uses \${CLAUDE_PLUGIN_ROOT} without double quotes (breaks on paths with spaces): $(printf '%s' "$UNQ" | head -1)"
      ERRORS=$((ERRORS + 1))
    fi
  fi
fi
...
if [ -f "$BUDGET_JSON" ]; then
  ...
  if [ -f "$PLUGIN_JSON" ] && jq -e 'has("outputStyles")' "$PLUGIN_JSON" >/dev/null 2>&1; then
    if [ ! -d "$PLUGIN_ROOT/output-styles" ]; then
      echo "FAIL: required directory missing: output-styles"
      ERRORS=$((ERRORS + 1))
    elif [ "$LIVE_STYLES" -eq 0 ]; then
      echo "FAIL: outputStyles declared but no .md files found under output-styles/"1
      ERRORS=$((ERRORS + 1))
    fi
  fi
...
elif [ ! -f "$PLUGIN_ROOT/.claude-plugin/ship-manifest.txt" ]; then
  # An installed copy (the shipped plugin/ tree) carries neither file: the budget gates repo
  # growth, which only the dev repo can have. The dev repo always has the manifest
  # (test-plugin-dist-current.sh), so a missing budget there still FAILs below.
  echo "SKIP: R8 surface budget — dev-repo gate, not applicable to a shipped tree"
else
