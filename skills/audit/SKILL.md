...
printf 'native auto-memory: %s  store=%s (%s files)\n' "${state:-unknown}" "${path:-?}" "${files:-0}"
```

(`state`/`path`/`files` are the detector's `key=value` output, consumed via
`eval`. See `sb_auto_memory_state` in `scripts/lib.sh`.) This is informational —
the audit never toggles auto-memory; `/second-brain:status` carries the
disable-offer details.
