...
EOF
  ...
  # Idempotent fill (user-invoked, mirroring the USER.md Intent fill above): add the standard
  # Charter to an existing card if it's missing — never rewrite the user's own content.
  if ! grep -q '^## Charter$' "$PCARD"; then
    cat >> "$PCARD" <<EOF
...
- Grasp the *why* behind the work — the vision behind the commands, not just executing them. That's where real usefulness lies.
EOF
    echo "persona-card.md present — added the missing ## Charter section"
  else
...
### 6d. Buddy statusline (opt-in, ask first)

The buddy is a `statusLine` renderer that shows what the plugin is doing under the input box
(goal · phase · last gate/delivery/capture event). Plugins cannot set `statusLine` themselves, so
**ask the operator** whether to install it; only on an explicit yes:

```bash
node "${CLAUDE_PLUGIN_ROOT}/mcp/dist/cli/sb-entry.bundle.js" buddy install
```

It backs up `~/.claude/settings.json`, chains any existing statusline as line 1, and is undone by
`/second-brain:buddy uninstall`. On no: mention `/second-brain:buddy install` for later and move on.
