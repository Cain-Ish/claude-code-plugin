...
The vector-deps health check runs `install-vector-deps.sh`, which creates the shared dir + symlink (or harvests an existing copy) on first run. Bumping the marker is sufficient.
