...
#
# SCOPE: this file exercises the CLI *bundle* path only (buildEpisodicIndex via
# episodic-index-cli.bundle.js). The episodicSearch MODULE paths — vector recall,
# multi-word text tokenization, and mode=both text fallback — are covered by vitest
# (mcp/src/tools/episodic-index.test.ts). The former shell TESTs 3-5 duplicated that
# coverage but required an on-the-fly esbuild of a search test-module plus Windows
# /c/ → C: ESM path shimming; both were removed here as pure test-suite cost.
set -eu
...
# state, NOT corruption) — skip the strict repair assertion instead of flaking.
# The repair path itself is covered by the vitest suite where the model exists.
# (Mirrors the test-episodic-index.test.ts skipIf-offline guard.)
if grep -qiE 'model load failed|fetch failed|could not locate|ENOTFOUND|getaddrinfo|transformers.*(fail|error)' "$TMP/run2.err"; then
  echo "  SKIP: embedding model unavailable (offline) — repair assertion skipped; $T rows intact, no poison"
else
