...
mkdir -p "$KNOWLEDGE_DIR/wiki/themes" "$KNOWLEDGE_DIR/wiki/misc" "$BRAIN_DIR"
# OLD (so age-protection does NOT apply) + unlinked theme page — only category protection can save it.
printf '%s\n' '---' 'title: Theme X' 'type: themes' 'generated: true' '---' '# theme x summary body here for length' \
  > "$KNOWLEDGE_DIR/wiki/themes/theme-x.md"
...
touch -d '40 days ago' "$KNOWLEDGE_DIR/wiki/themes/theme-x.md" "$KNOWLEDGE_DIR/wiki/misc/misc-x.md" 2>/dev/null \
...
theme_prot=$(printf '%s\n' "$OUT" | awk -F'\t' '$2=="theme-x"{print $5}')
misc_prot=$(printf '%s\n' "$OUT"  | awk -F'\t' '$2=="misc-x"{print $5}')

echo "$theme_prot" | grep -q 'PROTECT:category' || fail "old/unlinked theme page NOT category-protected (protflag='$theme_prot')"
pass "old/unlinked theme page is PROTECT:category (never a forget candidate)"
